...
def _only_painting(img, m):
    img = img.copy()
    img[m == 0] = 0
    x,y,w,h = cv2.boundingRect(m)
    return img[y:y+h,x:x+w], m[y:y+h,x:x+w]
...
def search_batch(museum_list:List[Path], query_list:List[Path], mask_list:List[Path], text_list, query_params, k=10) -> List[int]:
    ...
    with mp.Pool(processes=8) as p:
        ...
        # we crop the painting around the minimum rectangle containing the painting mask
        queries = [[_only_painting(img, m) for (img, m) in query] for query in queries]
